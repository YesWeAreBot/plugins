...
export class PGliteBuilder extends Builder {
  ...
  constructor(
    ...
    this.evalOperators = {
      ...
      $xor: (args) => {
        const type = Type.fromTerm(this.state.expr, Type.Boolean)
        if (Field.boolean.includes(type.type)) { return args.map((arg) => this.parseEval(arg, 'boolean')).reduce((prev, curr) => `(${prev} != ${curr})`) } else return `(${args.map((arg) => this.parseEval(arg, 'bigint')).join(' # ')})`
      },
